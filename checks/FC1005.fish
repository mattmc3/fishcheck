function FC1005 -d "Found bash-ism 'elif'"
    ...
    for fishfile in $argv
        ...
        while read --local line
            ...
            string match -rq -- '\belif\b' $line || continue # Quick skip check.
            ...
            if test "x_$tokens[1]" = x_elif
                set exitcode 1
                set position (string match -rng -- '\b(elif)\b' $line | string split ' ') || set position 0 0
                _fishcheck_report \
                    --file $fishfile \
